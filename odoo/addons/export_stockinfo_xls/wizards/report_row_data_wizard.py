...
class ReportRowDataWizard(models.TransientModel):
    _name = 'report.row.data.wizard'
    _description = 'Report Raw Data Wizard'
    ...
    def export_xls(self):
        ...
        return {'type': 'ir.actions.report',
                'data': {'model': 'report.row.data.wizard',
    ...
    def generate_info(self, sheet, workbook, value):
        ...
        for line in value:
            ...
            sheet.write(row, col_line + 9, partner_id and get_partner_province[partner_id]['code'] or '', format_data)
            sheet.write(row, col_line + 10, partner_id and get_partner_province[partner_id]['name'], format_data)
            sheet.write(row, col_line + 11, categ_id and get_product_category[categ_id]['name'], format_data)
            sheet.write(row, col_line + 12, product_id and get_product[product_id]['code'], format_data)
            sheet.write(row, col_line + 13, product_id and get_product[product_id]['name'], format_data)
            sheet.write(row, col_line + 14, line.get('product_uom_qty', ''), data_body_num_fmt)
            sheet.write(row, col_line + 15, line.get('price_unit', ''), data_body_num_fmt)
            sheet.write(row, col_line + 16, line.get('discount', ''), data_body_num_fmt)
